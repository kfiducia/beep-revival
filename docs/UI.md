...
| Gesture | Action |
|---|---|
| **Turn** | Volume. Clockwise = up. One LED of the ring per detent; the arc fills from the bottom (6 o'clock) clockwise. The **knob and the web slider are the volume authority** and always agree. |
...
> **AirPlay volume is one-way.** The knob and web slider set the Beep's volume;
> the AirPlay sender's slider (e.g. your iPhone) is **ignored** and won't change
> the Beep. This is deliberate: AirPlay-1 can't report a knob change back to an
> iOS sender, so honouring both would make the level jump whenever the phone
> re-asserted its slider. Use the knob (or the web slider) to set volume. (Two-way
> sync would require AirPlay-2, which this hardware can't run at acceptable CPU.)
